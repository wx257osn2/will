...
class d2d{
	...
	class ddr_base{
		...
		template<>
		class device_context<ID2D1GdiInteropRenderTarget>{HDC dc;ddr_base& t;public:device_context(D2D1_DC_INITIALIZE_MODE mode, ddr_base& t):t(t){t->GetDC(mode, &dc);}~device_context(){RECT r = {};t->ReleaseDC(&r);}operator HDC(){return dc;}};
	public:
		ddr_base():life(false){}
...
private:
	...
	template<typename F>
	void add_ddr(F&& f){ddrs.emplace_back(std::vector<std::shared_ptr<device_dependent_base>>{make_ddr<std::remove_pointer<decltype(f())>::type>(std::forward<F>(f))});}
	...
	class render_target_base{
		...
		U make_ddr(F&& f){factory.ddrs[index].emplace_back(d2d::make_ddr<std::remove_pointer<decltype(f())>::type>(std::forward<F>(f)));return U(factory, index, factory.ddrs[index].size()-1u);}
	public:
		...
		gradient_stop_collection make_gradient_stop_collection(std::vector<D2D1_GRADIENT_STOP>&& gradient_stops, D2D1_GAMMA gamma = D2D1_GAMMA_1_0, D2D1_EXTEND_MODE extm = D2D1_EXTEND_MODE_CLAMP){
			return this->make_ddr<gradient_stop_collection>([gradient_stops, gamma, extm, this]{return com_create_resource<ID2D1GradientStopCollection>([&](ID2D1SolidColorBrush** x){return get()->CreateGradientStopCollection(gradient_stops.data(), gradient_stops.size(), gamma, extra, x);});});
		}
		...
		bitmap make_bitmap(IWICFormatConverter* conv){
		...
		bitmap make_bitmap(WICConverter&& conv){
			return make_bitmap(conv.get());
		}
		bitmap_brush make_bitmap_brush(const bitmap& bm, const D2D1_BITMAP_BRUSH_PROPERTIES& extm, const D2D1_BRUSH_PROPERTIES& prop){
		...
		gdi_interop_render_target make_gdi_interop_render_target(){return this->make_ddr<gdi_interop_render_target>([this]{return com_create_resource<ID2D1GdiInteropRenderTarget>([&](ID2D1GdiInteropRenderTarget** x){return get()->QueryInterface(x);});});}
...
public:
	...
	d2d(d2d&&) = default;
	~d2d() = default;
	hwnd_render_target make_hwnd_render_target(HWND hwnd, const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED)), D2D1_PRESENT_OPTIONS op = D2D1_PRESENT_OPTIONS_IMMEDIATELY){add_ddr([hwnd, prop, op, this]{return com_create_resource<ID2D1HwndRenderTarget>([&](ID2D1HwndRenderTarget** x){const auto rc = [](HWND hwnd){RECT rc; GetClientRect(hwnd, &rc); return rc;}(hwnd);return factory->CreateHwndRenderTarget(prop, D2D1::HwndRenderTargetProperties(hwnd, D2D1::SizeU(rc.right - rc.left, rc.bottom - rc.top), op), x);});});return hwnd_render_target(*this, ddrs.size() - 1u);}
	template<typename Window>
	hwnd_render_target make_hwnd_render_target(Window&& w,  const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED)), D2D1_PRESENT_OPTIONS op = D2D1_PRESENT_OPTIONS_IMMEDIATELY){return make_hwnd_render_target(w.get_hwnd(), prop, op);}
	render_target make_wic_bitmap_render_target(IWICBitmap* bitmap, const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED), 0.0f, 0.0f, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE)){add_ddr([bitmap, prop, this]{return com_create_resource<ID2D1RenderTarget>([&](ID2D1RenderTarget** x){return factory->CreateWicBitmapRenderTarget(bitmap, prop, x);});});return render_target(*this, ddrs.size() - 1u);}
	template<typename Bitmap>
	render_target make_wic_bitmap_render_target(Bitmap&& bm, const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED), 0.0f, 0.0f, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE)){return make_wic_bitmap_render_target(bm.get(), prop);}
	render_target make_dxgi_surface_render_target(IDXGISurface* surface, const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED), 0.0f, 0.0f, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE)){add_ddr([surface, prop, this]{return com_create_resource<ID2D1RenderTarget>([&](ID2D1RenderTarget** x){return factory->CreateDxgiSurfaceRenderTarget(surface, prop, x);});});return render_target(*this, ddrs.size() - 1u);}
	template<typename Surface>
	render_target make_dxgi_surface_render_target(Surface&& sf, const D2D1_RENDER_TARGET_PROPERTIES& prop = D2D1::RenderTargetProperties(D2D1_RENDER_TARGET_TYPE_DEFAULT, D2D1::PixelFormat(DXGI_FORMAT_B8G8R8A8_UNORM, D2D1_ALPHA_MODE_PREMULTIPLIED), 0.0f, 0.0f, D2D1_RENDER_TARGET_USAGE_GDI_COMPATIBLE)){return make_dxgi_surface_render_target(sf.get(), prop);}
};
}